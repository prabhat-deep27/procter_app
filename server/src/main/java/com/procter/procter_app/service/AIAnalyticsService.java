...
public class AIAnalyticsService {
    ...
    private boolean filterByTimeRange(TestAttempt attempt, String timeRange) {
        ...
        switch (timeRange) {
            case "week":
                ...
                break;
            case "month":
                cutoff = Instant.now().minus(30, ChronoUnit.DAYS);
                break;
            case "semester":
                cutoff = Instant.now().minus(120, ChronoUnit.DAYS);
                break;
            default:
                cutoff = Instant.now();
                break;
    ...
    private StudentAnalyticsResponse.PerformanceTrend calculatePerformanceTrend(List<TestAttempt> attempts) {
        ...
        double changePercentage = 0.0; // Default to 0
        if (firstHalf > 0) {
            // Only perform the calculation if firstHalf is not zero
