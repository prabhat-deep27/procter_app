...
const StudentTestReviewPage = () => {
  ...
  const fetchStudentTestReview = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/student/test/${testId}/review`, {
        headers: {
  ...
  const getAnswerDisplay = (question, studentAnswer, correctAnswer) => {
    if (question.type === 'multiple_choice') {
      ...
      const studentIdx = studentAnswer;
      const correctIdx = correctAnswer;
      
      return {
        student: studentIdx !== null && studentIdx < options.length ? options[studentIdx] : 'No answer',
        correct: correctIdx !== null && correctIdx < options.length ? options[correctIdx] : 'No correct answer',
      ...
      return {
        student: studentIndices.map(idx => options[idx]).join(', ') || 'No answer',
        correct: correctIndices.map(idx => options[idx]).join(', ') || 'No correct answer',
        isCorrect: JSON.stringify(studentIndices.sort()) === JSON.stringify(correctIndices.sort())
      };
    } else {
      return {
        student: studentAnswer || 'No answer',
        correct: correctAnswer || 'No correct answer',
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{testReview.testTitle}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Subject</label>
                  <p className="text-lg font-semibold">{testReview.subject}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">{testReview.durationInMinutes} minutes</p>
                </div>
              </div>
            </Card>
          </div>
